...
export default class transitionScene extends Phaser.Scene {
    ...
    preload() {
        ...
        this.load.image('defeat', 'defeated.png');
    ...
    create() {
        ...
        const imgWidth = 1024;   // Original image width
        const imgHeight = 1024; // Original image height
        ...
        this.background = this.add.tileSprite(
            ...
            'defeat'      // Key of the loaded image
        )
        .setScale(scale)        // Apply the correct scale
        .setOrigin(0.5, 0.5);  // Center the sprite
    

        const panel = this.add.rectangle(50, 500, 500, 400, 0x333333 , 0.9)
        .setOrigin(0.5);
        this.potion = this.add.image(300, 500, 'tanya').setScale(0.8);

    const fullText = 'Finally I did it Bajin the Terror, The Destroyer your days of Spreading fear are over \n\n\nNo Wait What is going on !!! \n\nis this a Transformation !!  ';
    
    // Create an initially empty text object
     // Create an initially empty text object
     const text = this.add.text(150, 450, '', {
        font: '18px Arial',
        color: '#ffffff',
        align: 'center',
        wordWrap: {
            width: 230  // Maximum width in pixels before wrapping
        },
        align: 'left'
    }).setOrigin(0.5);
    ...
    const button = this.add.text(120, 650, 'Continue', {
